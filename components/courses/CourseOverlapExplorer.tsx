  ...
  return (
    <div className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-slate-50">
      <div className="flex items-center gap-2">
        ...
        <span className={`text-sm ${course.completed ? 'text-slate-900' : 'text-slate-500'}`}>{course.title}</span>
      </div>
        ...
        <div className="space-y-4">
            ...
            {filteredCourses.map(course => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-4 py-2.5 text-sm border-b border-slate-100 last:border-0 hover:bg-slate-50 flex justify-between items-center ${
                    selectedCourse === course.id ? 'bg-blue-50 text-blue-900' : 'text-slate-700'
                  }`}
                >
          ...
          {selectedCourse && (
            <div className="bg-white border border-slate-200 rounded-xl p-5">
              <h3 className="font-semibold text-slate-900 mb-1">{courseMap.get(selectedCourse)?.title}</h3>
                ...
                {coursePrograms.map(p => {
                  ...
                  return (
                    <Link
                      key={p.id}
                      href={`/programs/${p.id}`}
                      className="flex items-center justify-between p-3 rounded-lg border border-slate-100 hover:border-blue-200 hover:bg-blue-50 transition-colors"
                    >
                      <div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {pc?.completed
                          ? <span className="text-xs text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded">Done in this program</span>
                          : <span className="text-xs text-slate-500 bg-slate-50 px-2 py-0.5 rounded">Pending</span>
                        }
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-5">
                <h3 className="font-semibold text-slate-900 mb-3">
                  Shared Courses <span className="text-slate-400 font-normal">({sharedCourses.length})</span>
                </h3>
                {sharedCourses.length === 0 ? (
                  <p className="text-sm text-slate-400">No shared courses.</p>
                ) : (
                  <div className="space-y-1">
                    {sharedCourses.map(c => {
                      ...
                      return (
                        <div key={c.id} className="flex items-center justify-between py-1.5 px-2 rounded hover:bg-slate-50">
                          <span className="text-sm text-slate-800">{c.title}</span>
                          <div className="flex gap-2 text-xs shrink-0">
                            <span className={doneInA ? 'text-emerald-600' : 'text-slate-400'}>A: {doneInA ? '✓' : '○'}</span>
                            <span className={doneInB ? 'text-emerald-600' : 'text-slate-400'}>B: {doneInB ? '✓' : '○'}</span>
                          </div>
                        </div>
                      );
                    })}
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={label} className="bg-white border border-slate-200 rounded-xl p-4">
                    <h4 className="text-xs font-semibold text-slate-500 uppercase mb-2">{label}</h4>
                    <div className="space-y-1">
                      {courses.map(pc => {
                        const c = courseMap.get(pc.courseId);
                        return c ? (
                          <div key={pc.courseId} className="flex items-center gap-2 text-xs text-slate-700">
                            {pc.completed
